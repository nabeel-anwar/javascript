...
const formatNumber = (value, locale, currency) => Intl.NumberFormat(locale, {
  style: 'currency',
  currency: currency
}).format(value);
...
const logOutSession = () => {
  ...
  const tick = () => {
    labelTimer.textContent = `${`${Math.trunc(time / 60)}`.padStart(2, 0)}:${`${Math.trunc(time % 60)}`.padStart(2, 0)}`;
    if (time === 0) {
      clearInterval(timer);
  ...
  tick(); // Calling immediately bcz interval run after 1s
  ...
  movs.forEach(function (mov, i) {
    ...
    const html = `
      ...
      <div class="movements__date">${displayDate}</div>
        ...
        <div class="movements__value">${formatNumber(mov, acc.locale, acc.currency)}</div>
      </div>
    `;
